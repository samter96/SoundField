    ...
    if let Ok(RawWindowHandle::Win32(_w)) = handle.window_handle().map(|h| h.as_raw()) {
        match item {
            DragItem::Files(files) => {
                ...
                // ── SoundField 수정 ①: 경로를 정규화하지 않는다 ────────────────
                // 원래 코드: `paths.push(dunce::canonicalize(f)?)`
                // 정규화하면 Windows 의 verbatim 경로(`\\?\...`)가 나온다. 드라이브 문자
                // 경로는 dunce 가 접두사를 떼주지만, **네트워크 드라이브는
                // `\\?\UNC\서버\공유\...` 형태로 남는다.** 이 형태를 ILCreateFromPathW 가
                // 받지 않아 아래에서 None 이 되고, 원래 코드는 그걸 unwrap 해서 패닉했다 —
                // 패닉이 COM 드래그 경로를 거슬러 올라가므로 **앱이 통째로 죽었다.**
                // 실측: `Y:\...`(136자) → `\\?\UNC\victor\...`(168자) → null.
                //       원래 경로 그대로면 NAS 파일 40/40 성공, 정규화하면 전부 실패.
                // 호출부가 항상 절대 경로를 넘기므로 정규화는 필요하지 않다.
                // (원본 SoundField(PyQt) 도 경로를 변환하지 않고 그대로 넘겼다.)
                let paths: Vec<PathBuf> = files;

                // ── SoundField 수정 ②: unwrap 금지 ─────────────────────────────
                // 원래 코드: `get_file_data_object(&paths).unwrap()`
                // 실패를 오류로 돌려준다. 여기서 패닉하면 앱이 죽는다.
                let Some(data_object) = get_file_data_object(&paths) else {
                    return Err(crate::Error::FailedToStartDrag);
                };
                let data_object: IDataObject = data_object;
            ...
            DragItem::Data { .. } => {
                ...
                // SoundField 수정 ③: unwrap 금지 (위와 같은 이유)
                let Some(data_object) = get_file_data_object(&paths) else {
                    return Err(crate::Error::FailedToStartDrag);
                };
                let data_object: IDataObject = data_object;
...
fn get_file_data_object(paths: &[PathBuf]) -> Option<IDataObject> {
    unsafe {
        // SoundField 수정 ④: 실제 패닉 지점이었다 (원래 `...unwrap()`, mod.rs:370).
        // 셸이 경로를 다루지 못하면 None 을 돌려준다 — 호출부가 오류로 처리한다.
